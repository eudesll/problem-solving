/* 
	HackerRank
	Problem Name: s10-binomial-distribution-2
*/
...
public class Solution {
	...
	public static double  binomial(int n, int x, double p) {
		...
		return (combination_N / (combination_D1 * combination_D2)) * (Math.pow(p, x)) * (Math.pow((1 - p), (n - x)));
	}
	...
	public static void main(String[] args) {
		...
		double rejected = scan.nextDouble();
		int n = scan.nextInt();

		double p = rejected / 100;
		...
		System.out.println(Math.round((combination2 + combination3 + combination4 + combination5 + combination6 + combination7 + combination8 + combination9 + combination10) * 1000.0) / 1000.0);
	}
}